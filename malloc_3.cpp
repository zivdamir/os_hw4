...
void* srealloc(void* oldp, size_t size){
    ...
    if(block->is_mmap==false) {
        MallocData closest_block_behind = find_closest_block_behind((MallocData) oldp);
        MallocData closest_block_in_front = find_closest_block_behind((MallocData) oldp);
        ...
        bool three_blocks_are_enough = (size <= block_behind_size_with_meta + block_behind_size_with_meta + block_size);
        ...
        if (can_use_block_behind) {
            ...
            if (was_expanded || the_block_behind_is_big_enough) {
                ...
                MallocData merged_block = merge_blocks_into_block_one(block, closest_block_behind);
